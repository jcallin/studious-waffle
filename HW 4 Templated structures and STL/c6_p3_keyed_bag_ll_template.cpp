...
Name:Julian Callin
SUID:W1012798
Chapter 6 project 3: Dynamic array keyed bag template
*/
#include <iostream>
...
class keyed_bag{
        ...
        bool insert(item y, item key){
                if(size() >= capacity){
                        item* temp_data = new item[2*capacity];
                        item* temp_keys = new item[2*capacity];
        ...
        bool erase_item(const item key){
                if(current < 0)
                        return false;
                if(current == 0){
                        current = -1;
        ...
        keyed_bag<item>& operator-(const keyed_bag<item>& b){
                int a_index;
                int b_index;
                item match;
